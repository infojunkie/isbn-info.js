...
import meow from 'meow'; // https://stackoverflow.com/a/54577682/209184
...
function parseInput(input) {
  ...
  const isbn = filename.match(/\d{13}|\d{10}|\d{9}X/i); // Ignore invalid ISBN strings.
...
function addIsbnIfNotThere(isbn, book) {
  ...
  [{
    type: 'ISBN_10',
    identifier: () => isbn.isbn10
  }, {
    type: 'ISBN_13',
    identifier: () => isbn.isbn13
  }].forEach(i => {
...
function sanitizeFilename(title) {
  ...
  const truncated = Buffer.byteLength(title) <= 255 ? title : `${title.slice(0, truncation)}${ellipsis}${title.slice(-truncation)}`; // Copied from https://github.com/parshap/node-sanitize-filename because we do our own truncation.
