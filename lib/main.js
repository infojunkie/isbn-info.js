...
OPTIONS['_'].forEach(function (input) {
  ...
  _nodeIsbn["default"].provider(['isbndb', 'google', 'worldcat']).resolve(isbn.codes.source, function (err, book) {
    if (err) {
...
function formatBook(input, book, format, options) {
  ...
  return options['s'] ? (0, _sanitizeFilename["default"])(result, {
    replacement: ' '
  }).trim().replace(/(["\s'$`\\])/g, '\\$1') : result;
}