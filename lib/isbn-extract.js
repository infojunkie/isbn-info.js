...
var regexIsbn = /\bISBN(?:-1[03])?:? ((?=[0-9X]{10}\b|(?=(?:[0-9]+[- ]){3})[- 0-9X]{13}\b|97[89][0-9]{10}\b|(?=(?:[0-9]+[- ]){4})[- 0-9]{17}\b)(?:97[89][- ]?)?[0-9]{1,5}[- ]?[0-9]+[- ]?[0-9]+[- ]?[0-9X]\b)/g;
var regexIssn = /\bISSN:? ((?:\d{4})-?(?:\d{3})(?:[\dX]))\b/g;

var matches = _toConsumableArray(text.matchAll(OPTIONS.flags['type'] === 'isbn' ? regexIsbn : regexIssn)).map(function (match) {
  return match[1];
}).reduce(function (matches, match) {
  if (OPTIONS.flags['type'] === 'isbn') {
    var p = _isbn["default"].parse(match);
