...
function _resolveOclcClassify(isbn, options) {
    const standardize = function standardize(classifyData) {
        const standardBook = {
            'title': '',
            'publishedDate': '',
            'authors': [],
        ...
        if (classifyData.work) {
            ...
            if (work['@_author']) {
                standardBook.authors.push(work['@_author']);
            }
...
class Isbn {
    ...
    provider(providers) {
        ...
        if (!providersSupported)
            throw new Error('Please pass in supported providers.');
...
function formatBook(input, book, OPTIONS) {
    ...
    const result = Object.keys(replacements).reduce((result, pattern) => {
        ...
        return result.replace(regex, function () {
            try {
                ...
                if (!field) {
                    if (!OPTIONS.flags['quiet'])
                        console.warn(`Pattern ${pattern} empty for ${input}`);
                    return 'Unknown';
                }
                return field;
            }
                ...
                if (e instanceof TypeError) {
                    if (!OPTIONS.flags['quiet'])
                        console.warn(`Pattern ${pattern} broke for ${input}`);
                    return 'Unknown';
                }
                else {
                    throw e;
    ...
    if (result === OPTIONS.flags['format'].replace(empty, 'Unknown'))
        return null;
    return OPTIONS.flags['sanitize'] ? sanitizeFilename(result + path.extname(input)) : result;
}
//# sourceMappingURL=isbn-format.js.map